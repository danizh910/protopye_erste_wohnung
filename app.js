...
function hasStandingOrderRequiredFields(item) {
...
function isStandingOrderItemComplete(item) {
  return hasStandingOrderRequiredFields(item) && item.confirmed;
}
...
function screenStandingOrder() {
  ...
  return layout("Daueraufträge", `<div class="card stack"><h2>Daueraufträge einrichten</h2><p class="subtext">Du kannst mehrere Daueraufträge erfassen. Diese kannst du später im E-Banking ansehen, löschen und editieren.</p>${numberInput("so-amount", d.monthlyAmount || "", "Monatlicher Betrag (€)")}<label>Empfänger<input id="so-recipient" value="${d.recipient}" placeholder="z.B. Vermietung Muster AG" /></label><label>IBAN<input id="so-iban" value="${d.iban}" placeholder="CH93...." /></label><label>Ausführungstag (1-28)<input type="number" min="1" max="28" id="so-day" value="${d.executionDay || 1}" /></label><label>Startdatum<input type="date" id="so-start" value="${d.startDate}" /></label><label>Zweck<input id="so-purpose" value="${d.purpose}" /></label><button class="secondary" data-action="save-standing-order">Daten speichern</button><button class="primary" data-action="confirm-standing-order" ${hasStandingOrderRequiredFields(d) ? "" : "disabled"}>Dauerauftrag bestätigen</button><button class="primary" data-action="add-standing-order" ${isStandingOrderItemComplete(d) ? "" : "disabled"}>Als weiteren Dauerauftrag erfassen</button></div><div class="card"><h3>Erfasste Daueraufträge</h3><ul>${items || "<li>Noch keine Daueraufträge erfasst.</li>"}</ul><p class="subtext">Hinweis: Im E-Banking kannst du Daueraufträge ansehen/löschen/editieren.</p></div>`);
}

function insuranceScore(option, criterion) {
  const coverage = Number(option.coverage) || 0;
  const premium = Number(option.annualPremium) || 0;
  const deductible = Number(option.deductible) || 0;

  const weights = {
    balanced: { coverage: 100, premium: 1.0, deductible: 0.4 },
    premium: { coverage: 65, premium: 1.6, deductible: 0.2 },
    deductible: { coverage: 75, premium: 0.8, deductible: 1.2 },
    coverage: { coverage: 140, premium: 0.7, deductible: 0.3 }
  };

  const selected = weights[criterion] || weights.balanced;
  return (coverage * selected.coverage) - (premium * selected.premium) - (deductible * selected.deductible);
}
...
function screenInsurance() {
  ...
  const criterion = state.insurance.priorityCriterion || "balanced";
  const best = [...selected].sort((a, b) => insuranceScore(b, criterion) - insuranceScore(a, criterion))[0];
  const worst = [...selected].sort((a, b) => insuranceScore(a, criterion) - insuranceScore(b, criterion))[0];
  const savings = best && worst ? Math.max(0, Number(worst.annualPremium) - Number(best.annualPremium)) : 0;
  return layout("Versicherungen", `<div class="card stack"><h2>Versicherungen logisch entscheiden</h2><p class="subtext">Jetzt passiert beim Vergleich etwas: wir berechnen eine nachvollziehbare Empfehlung aus Prämie, Selbstbehalt und Deckung.</p><label>Bis wann vergleichst du Angebote?<input type="date" id="insurance-compare" value="${state.insurance.compareBy}" /></label><label>Wichtigstes Kriterium<select id="insurance-priority"><option value="">Bitte auswählen</option><option value="balanced" ${state.insurance.priorityCriterion === "balanced" ? "selected" : ""}>Ausgewogen</option><option value="premium" ${state.insurance.priorityCriterion === "premium" ? "selected" : ""}>Niedrige Prämie</option><option value="deductible" ${state.insurance.priorityCriterion === "deductible" ? "selected" : ""}>Niedriger Selbstbehalt</option><option value="coverage" ${state.insurance.priorityCriterion === "coverage" ? "selected" : ""}>Hohe Deckung</option></select></label><button class="secondary" data-action="save-insurance">Vergleich auswerten</button>${state.insurance.recommendation ? `<p><strong>Empfehlung:</strong> ${state.insurance.recommendation}${savings ? ` · Potenzielle Ersparnis: ${savings} CHF/Jahr` : ""}</p>` : ""}</div>${options}<button class="primary" data-action="toggle-task" data-task-id="insurance" ${isInsuranceComplete() ? "" : "disabled"}>Schritt abschliessen</button>`);
}
...
document.addEventListener("click", (event) => {
  ...
  if (action === "confirm-standing-order") {
    state.standingOrders.draft.confirmed = hasStandingOrderRequiredFields(state.standingOrders.draft);
    saveState();
    return setToast(state.standingOrders.draft.confirmed ? "Dauerauftrag bestätigt" : "Dauerauftrag ist noch unvollständig");
  }
  ...
  if (action === "save-insurance") {
    ...
    if (selected.length && state.insurance.priorityCriterion) {
      const best = [...selected].sort((a, b) => insuranceScore(b, criterion) - insuranceScore(a, criterion))[0];
      const labels = { balanced: "ausgewogen", premium: "niedrige Prämie", deductible: "niedriger Selbstbehalt", coverage: "hohe Deckung" };
      state.insurance.recommendation = `${best.title} passt am besten zum Kriterium: ${labels[criterion]}`;
    } else {
